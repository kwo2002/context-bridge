import type { ApiClient, SessionCompareData } from "../api-client.js";

export async function handleGetSessionCompare(
  apiClient: ApiClient,
  args: { sessionId1: string; sessionId2?: string }
): Promise<string> {
  const data = await apiClient.compareSessions(args.sessionId1, args.sessionId2);
  return formatSessionCompare(data);
}

function formatSessionCompare(data: SessionCompareData): string {
  const lines: string[] = [];
  ...
  if (data.session1.keyDecisions.length > 0) {
    ...
    for (const d of data.session1.keyDecisions) { lines.push(`  - ${d}`); }
  }
  lines.push("");
  ...
  if (data.session2.keyDecisions.length > 0) {
    lines.push(`- **Key decisions:**`);
    for (const d of data.session2.keyDecisions) { lines.push(`  - ${d}`); }
  }
  lines.push("");
